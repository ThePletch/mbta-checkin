...
(function() {
  ...
  this.Helpers = (function() {
    ...
    Helpers.mergePredictions = function(predictions) {
      var i, key, len1, mergeDirectionPair, mergePair, mergePredictionPair, mergeRoutePair, newPredictions, prediction, results, val;
      mergePair = function(pair, subarrayExtractor, keyExtractor, mergeFunction) {
        var currentValue, element, i, key, len1, newSet, ref, results, val;
        newSet = {};
        ref = subarrayExtractor(pair[1]);
        for (i = 0, len1 = ref.length; i < len1; i++) {
          element = ref[i];
      ...
      mergeRoutePair = function(route, secondRoute) {
        ...
        return _.extend(route, {
          directions: mergePair([route, secondRoute], subarrExtractor, keyExtractor, mergeDirectionPair)
        });
      };
      ...
      newPredictions = {};
      for (i = 0, len1 = predictions.length; i < len1; i++) {
  ...
  this.Marker = (function() {
    function Marker(lat1, lng1, category) {
      this.lat = lat1;
      this.lng = lng1;
  ...
  this.LocationMarker = (function(superClass) {
    extend(LocationMarker, superClass);

    function LocationMarker(lat1, lng1) {
      this.lat = lat1;
      this.lng = lng1;
      this.render = bind(this.render, this);
  ...
  this.Stop = (function(superClass) {
    extend(Stop, superClass);

    function Stop(id1, name1, lat1, lng1, category) {
      this.id = id1;
      this.name = name1;
      this.lat = lat1;
      this.lng = lng1;
    ...
    Stop.prototype.onClick = function() {
      ...
      return async.map(stopAndChildren, (function(stopId, callback) {
        ...
        return function(err, predictions) {
          var result;
          if (err) {
  ...
  this.Vehicle = (function(superClass) {
  ...
  this.LiveTrain = (function(superClass) {
    extend(LiveTrain, superClass);

    function LiveTrain(id1, line1, destination, lat, lng, bearing) {
      this.id = id1;
      this.line = line1;
  ...
  this.Route = (function() {
    function Route(id1, name1, mode, stops, vehicles1) {
      this.id = id1;
      this.name = name1;
      this.mode = mode;
      this.stops = stops;
    ...
    Route.byId = function(id, callbacks) {
      if (Helpers.cache.routes[id]) {
        return callbacks.success(Helpers.cache.routes[id]);
      } else {
        return Mbta.getRoute(id, {
          success: function(result) {
            return callbacks.success(Route.fromRawApi(result));
          }
        });
      }
    };
    ...
    Route.fromRawApi = function(api) {
      var base, name1, route;
      route = api.data;
      return (base = Helpers.cache.routes)[name1 = route.id] != null ? base[name1] : base[name1] = new Route(route.id, route.attributes.name, "Subway");
    };
    ...
    Route.prototype.render = function(renderStops) {
      Route.getShapes(this.id, (function(_this) {
    ...
    Route.prototype.destroy = function() {
      var ref;
      if ((ref = this.paths) != null) {
        ref.map(function(path) {
